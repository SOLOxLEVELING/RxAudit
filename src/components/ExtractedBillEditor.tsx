  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 text-zinc-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-zinc-800">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">
            Review or edit extracted items to fix OCR reading before auditing.
          </p>
        </div>
        <button
          onClick={onAudit}
          disabled={isAuditing || bill.lineItems.length === 0}
          className="inline-flex items-center justify-center gap-1.5 px-4 py-2 text-xs font-semibold rounded bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:pointer-events-none text-white transition-colors cursor-pointer"
        >
      ...
      <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
        <div>
          <label className="block text-[11px] text-zinc-400 mb-1">Pharmacy / Store</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-[11px] text-zinc-400 mb-1">Bill / Invoice #</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-[11px] text-zinc-400 mb-1">Bill Date</label>
          <input
            type="date"
            value={bill.date || ""}
              ...
              <tr key={idx} className="group hover:bg-zinc-800/30">
                <td className="py-2 pr-2">
                  <input
                    ...
                    className="w-full bg-zinc-950 border border-zinc-800 rounded px-2 py-1 text-zinc-100 text-xs font-medium focus:outline-none focus:border-zinc-600"
                  />
                </td>
                <td className="py-2 px-2">
                  <input
                    ...
                    className="w-full bg-zinc-950 border border-zinc-800 rounded px-2 py-1 text-zinc-200 text-right text-xs font-mono focus:outline-none focus:border-zinc-600"
                  />
                </td>
                <td className="py-2 px-2">
                  <input
      ...
      {bill.lineItems.length === 0 && (
        <div className="py-8 text-center text-xs text-zinc-400 border border-dashed border-zinc-800 rounded mt-2">
          No line items on this bill. Add rows manually or select a sample bill.
        </div>
      )}

      {/* Table Footer Actions */}
      <div className="mt-3 flex items-center justify-between pt-2 border-t border-zinc-850">
        <button
          onClick={handleAddRow}
          className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border border-zinc-700 transition-colors"
        >
